...
const GallerySection = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActive((prev) => (prev === galleryData.length - 1 ? 0 : prev + 1));
    }, 4200);
  ...
  // Build ordered array: active first, then next cards in order
  const orderedIndexes = Array.from({ length: galleryData.length }, (_, i) =>
    (active + i) % galleryData.length
  );

  return (
    <section
      className={`gtSpatialGallerySection${visible ? " gtVisible" : ""}`}
      ref={sectionRef}
    >
      {/* Grid */}
      ...
      <div
        className="gtSpatialDynamicGlow"
        style={{
          background: `radial-gradient(circle, ${item.glow}55, transparent 70%)`,
        }}
      />
      ...
      {/* Header */}
      <div className="gtSpatialTop gtSpatialTopShow">
      ...
      {/* Main layout: info panel + stacked cards */}
      <div className="gtMainLayout">

        {/* LEFT — Glass info panel, always visible */}
        <div className="gtInfoPanel">
          <div className="gtInfoInner">
            <span className="gtInfoTag">{item.tag}</span>
            <h3 className="gtInfoTitle">{item.title}</h3>
            <p className="gtInfoDesc">{item.desc}</p>

            <div className="gtInfoDivider" />

            <div className="gtInfoStat">
              <span className="gtInfoStatNum">{item.stat}</span>
              <span className="gtInfoStatLabel">Documented Impact</span>
            </div>

            <button className="gtInfoBtn">
            ...
            <div className="gtDots">
              {galleryData.map((_, i) => (
                <button
                  key={i}
                  className={`gtDot${i === active ? " gtDotActive" : ""}`}
                  onClick={() => setActive(i)}
        ...
        {/* RIGHT — Stacked deck */}
        <div className="gtDeckWrap">
          {/* Render in reverse so front card is on top */}
          {[...orderedIndexes].reverse().map((dataIdx, revPos) => {
            const pos = orderedIndexes.length - 1 - revPos; // 0 = front
            const isFront = pos === 0;
            return (
              <div
                key={dataIdx}
                className="gtDeckCard"
                style={{
                  "--pos": pos,
                  zIndex: galleryData.length - pos,
                  opacity: isFront ? 1 : Math.max(0.18, 1 - pos * 0.22),
                  filter: isFront ? "none" : `blur(${pos * 1.2}px) brightness(${1 - pos * 0.18})`,
                  transform: `
                    translateX(${pos * 38}px)
              ...
              >
                <img
                  ...
                  alt={galleryData[dataIdx].title}
                  className="gtDeckImg"
                />
                <div className="gtDeckOverlay" />

                {isFront && (
                  <div className="gtDeckMetric">
                    {galleryData[dataIdx].stat}
                  </div>
                )}
              </div>
            );
