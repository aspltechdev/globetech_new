...
const galleryData = [
  ...
  },
  {
    ...
    desc: "Preparing students for successful careers through training, mentorship, and industry engagement.",
    stat: "1,200+",
    statLabel: "Placements Facilitated",
    tag: "CAREERS",
    glow: "#4ecdc4",
  },
  {
...
const GallerySection = () => {
  ...
  // stack order: active = front (pos 0), rest behind in order
  const stackOrder = Array.from({ length: galleryData.length }, (_, i) =>
    (active + i) % galleryData.length
  );

  return (
    <section
      className={`gtGallery${visible ? " gtGalleryVisible" : ""}`}
      ref={sectionRef}
    >
      {/* background grid */}
      ...
      <div
        className="gtGlow"
        style={{ background: `radial-gradient(circle, ${item.glow}44, transparent 70%)` }}
      />

      {/* floating bg words */}
      <div className="gtBgWords" aria-hidden>
        <span>IMPACT</span>
        <span>CHANGE</span>
        <span>HUMANITY</span>
      </div>
      ...
      {/* section heading */}
      <div className="gtHeadWrap">
        <span className="gtEyebrow">GALLERY EXPERIENCE</span>
        <h2 className="gtHeading">Every Moment<br />Creates Impact.</h2>
        <p className="gtSub">Real stories. Real transformation. Real people shaping a better future.</p>
      </div>
      ...
      <div className="gtRow">
        ...
        <div className="gtPanel">
          <div className="gtPanelGlass">
            <span className="gtPanelTag">{item.tag}</span>
            <h3 className="gtPanelTitle" key={active + "t"}>{item.title}</h3>
            <p className="gtPanelDesc" key={active + "d"}>{item.desc}</p>

            <div className="gtPanelRule" />

            <div className="gtPanelStat" key={active + "s"}>
              <span className="gtStatBig">{item.stat}</span>
              <span className="gtStatSmall">{item.statLabel}</span>
            </div>

            <button className="gtPanelBtn">
              Explore Story <ArrowUpRight size={15} />
            </button>

            {/* dots */}
            <div className="gtDots">
              {galleryData.map((_, i) => (
                <button
                  key={i}
                  className={`gtDot${i === active ? " gtDotOn" : ""}`}
                  onClick={() => setActive(i)}
        ...
        {/* RIGHT: stacked deck — front card right, rest peeking LEFT + UP */}
        <div className="gtDeck">
          {stackOrder.map((dataIdx, pos) => {
            // pos 0 = front (rightmost), pos 1,2,3,4 = behind peeking left
            const isFront = pos === 0;
            const total = galleryData.length;

            // each card behind shifts LEFT and UP, shrinks, dims
            const xShift   = pos * -52;   // peek left
            const yShift   = pos * -22;   // peek upward
            const scale    = 1 - pos * 0.07;
            const rotate   = pos * -2.8;  // slight tilt
            const opacity  = isFront ? 1 : Math.max(0.15, 1 - pos * 0.2);
            const blurVal  = isFront ? 0 : pos * 1.5;
            const bright   = isFront ? 1 : Math.max(0.4, 1 - pos * 0.15);
            const zIdx     = total - pos;
            ...
            return (
              <div
                ...
                style={{
                  zIndex: zIdx,
                  transform: `translateX(${xShift}px) translateY(${yShift}px) scale(${scale}) rotate(${rotate}deg)`,
                  opacity,
                  filter: `blur(${blurVal}px) brightness(${bright})`,
                  cursor: isFront ? "default" : "pointer",
                }}
                onClick={() => !isFront && setActive(dataIdx)}
              >
                <img src={galleryData[dataIdx].image} alt={galleryData[dataIdx].title} className="gtCardImg" />
                <div className="gtCardOverlay" />

                {/* metric pill — only on front */}
                {isFront && (
                  <div className="gtCardMetric">{item.stat} — {item.statLabel}</div>
                )}
              </div>
            );
          })}
